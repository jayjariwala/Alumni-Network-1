...
#myprofile,#searchmem,#gallery,#privmsg,#makeapp,#reportuser,#mod_req,#user_req{
...
<div class="content">
	<form id="form1" name="form1" method="post" action="ForumCreateSERVLET">
	
	<div style="margin-left:10px;margin-right:10px;">
	<div class="newpost-content" id="newpost-content1" style="border-top-right-radius: 50px;">
        <div id="newforum">
        <div style="float:left;width:70%;" class="newpost-content2">
          ...
          <h4 style="margin-left:40px;margin-right:10px;display:inline">Subject :</h4>
          <input type="text" name="f1" class="foruminput" style="margin-left:31px;margin-right:10px;width:70%;" placeholder="Enter Subject/Topic Name ">
          <br />
          <h4 style="margin-left:40px;margin-right:10px;display:inline">Description :</h4>
          <textarea name="f2" class="forumtextarea" rows="4" style="margin-right:21px;width:70%;vertical-align: top;resize:none;" placeholder="Enter your Description/Message"></textarea>
        </div>
...
$(document).ready(function() {
	$.post('PermisionDrpSERVLET',{},function(responseText) {
		if(responseText != "null")
         ...
         <select style="margin-top:20px;" class="forumselect" name="f3" id="inputPerm">
          ...
          <br /><br /><br />
          <button id="post-btn" type="submit" name="button" id="button" value="Submit" onclick="return val(form1)" >Post</button>
      	</div>
        </div>
        
        <div style="margin-top:225px;">
          <button type="button" class="newpost-btn" id="newpost-btn1"> New Post </button>
        </div>
	</div>
	</div>
	 </form>  		
...
<!------------------ Forum Posts starts --------------->
	...
	<div class="forum" id="i-fourm" style="border-top-right-radius: 50px;border-bottom-left-radius: 50px;margin-left:10px;margin-right:10px;margin-bottom:20px;">
            ...
            <tbody class="selectforum">
            	...
            	%> 	
    					<tr id="posted-forum<%=i%>" class="forum">
    		                ...
    		                <form action="RemoveFavForumSERVLET" name="remfavform<%=i%>" method="post">
								<input type="hidden" value="comment<%=i%>" name="cmt_ID" />
								<input type="hidden" value="<%= fb1.getF_id() %>" name="f_id"/>
								<% String favformname="remfavform"+i; %>
    		                	<a onclick="$('form[name=<%=favformname%>]').submit();"><img src="images/pinned.png" height="30" width="40" class="pin1"> <img src="images/pinned.png" height="28" width="40" class="pinned1" style="display: none;"></a>
    		                </form>
    		                </td>
    		                <%}else{ %>
    		                <td style="text-align:center;">
    		                
    		                <form action="FavForumSERVLET" name="favform<%=i%>" method="post">
											<input type="hidden" value="comment<%=i%>" name="cmt_ID" />
											<input type="hidden" value="<%= fb1.getF_id() %>" name="f_id"/>
											<% String favformname="favform"+i; %>
    		                	<a onclick="$('form[name=<%=favformname%>]').submit();"><img src="images/pin.png" height="20" width="30" class="pin1"> <img src="images/pinned.png" height="28" width="40" class="pinned1" style="display: none;"></a>
    		                </form>
    		                </td>
    						<%} %>
    					</tr>
    					<tr id="comment<%=i%>">
    						...
    						<table width="100%" border="1">
    							<tr>
    								...
    								<input type="hidden" value="<%= fb1.getF_id() %>" name="f_id"/>
    								<% if(lb.getM_id() == fb1.getCreator_id()){ %>
	  										<center><button id="btn" class="btnStyleDel"  type="submit" name="delete-btn"></button></center>
    								<% } %>
    								</form>
    								<%
    								}
    							...
    							%>
              					<tr>
                								<td style="color:#660066;"><b><%= cb.getFname() + " " +cb.getLname() +":"%></b><br/><br/>Date :<%= fmt.format(cb.getCreated_DateTime()) %><br/>Time :<%= fmt1.format(cb.getCreated_DateTime()) %></td>
                								<td colspan="5">
												<table width="100%">
												<tr>
    												<td colspan="5">
                								...
                								<td style="width:70px;">
											<%if(cb.isLiked()){ %>
											<center><img src="images/rated.png" style="width: 26px; height: 26px;"><b Style="font-size: 30px;color: #F25C27;" ><%= "  "+cb.getRate() %></b></center>
											<%}else{ %>
											<form action="LikeCommentSERVLET" name="likeform<%= i+""+k %>" method="post">
											<input type="hidden" value="comment<%=i%>" name="cmt_ID" />
											<input type="hidden" value="<%= cb.getC_Id() %>" name="c_id"/>
											<% String formname="likeform"+i+""+k; %>
											<center><a onclick="$('form[name=<%=formname%>]').submit();" ><img src="images/rate.png" id="like" style="width: 26px; height: 26px;"/><b Style="font-size: 30px;color: #F25C27;" ><%= "  "+cb.getRate() %></b></a></center>
											</form>
											<%} %>
											<br/>
											...
											<form name="dlt" action="CommentDeleteSERVLET" method="post">
    											<input type="hidden" value="<%= cb.getC_Id() %>" name="c_id"/>
    											<input type="hidden" value="comment<%=i%>" name="cmt_ID" />
												<center><button id="btn" class="cmtbtnStyleDel"  type="submit" name="cmt-delete-btn"></button></center>
											</form>
											<%} %>
											</td>
              								</tr>
              					...
              					<tr>
                					...
                					<td colspan="6">
										<table width="100%">
											<tr class="pending">
												<form action="CommentSERVLET" method="post" id="CommentForm<%=i%>">
												<td>
													<textarea rows="3" style="resize:none;design:inline;" id="addcomment<%=i%>" name="addcomment" cols="100"></textarea><br />
													<label id="EmptyCmt<%=i%>">This Field is Required.</label>
												</td>
												<td>
													<input type="hidden" value="<%= fb1.getF_id() %>" name="f_Id" /> 
													<input type="hidden" value="comment<%=i%>" name="cmt_ID" />
													<input type="submit" class="btnStyle" id="post-btn<%=i%>" value="POST" />
												</td>
												</form>	
											</tr>
										</table>                			
